...
print(c/10000)
num_sides = 6
outcomes = np.arange(2, 2 * num_sides + 1)
pmf = np.array([np.count_nonzero(np.random.randint(1, num_sides + 1, 10000) + np.random.randint(1, num_sides + 1, 10000) == i) / 10000 for i in outcomes])
plt.stem(outcomes, pmf, basefmt=' ', linefmt='k-', markerfmt='ro', use_line_collection=True)
plt.title("PMF of the Sum of Two Dice")
plt.xlabel("Sum of Two Dice")
plt.ylabel("Probability")
plt.grid(axis='y', linestyle='--', alpha=0.7)
plt.xlim(2, 2 * num_sides)
